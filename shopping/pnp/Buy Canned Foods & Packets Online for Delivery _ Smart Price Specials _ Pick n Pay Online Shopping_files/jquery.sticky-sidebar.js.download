$(document).ready(function() {
	...
	var footerHeight = $('.main-footer').outerHeight() + 33;
	var desktopWidthBreakpoint = 1024;
	$.stickysidebarscroll(".stickThis > .row", { offset: { top: headerHeight, bottom: footerHeight }, desktopWidthBreakpoint: desktopWidthBreakpoint });
});
    ...
    $.extend({
        ...
        "stickysidebarscroll": function (el, config) {
            if (config && config.offset) {
                ...
                config.offset = {bottom: 10, top: 0};
            }

            var el = $(el);

            if ($(window).width() > config.desktopWidthBreakpoint) {
	            if (el && el.offset()) {
